#!/usr/bin/env python3
"""
WQED Mean-Field Example for Local PC (sequential, no parallelization)
"""
...
def main():
    ...
    output_dir = Path("results/wqed_pc")
    ...
    for r in r_vals:
        Omega = r * omc
        Omega_n = Omega * np.exp(2j * np.pi * z)
        sol = solve_system_prop(nz, gammap, Dnm, Gnm, Omega_n, t_span, y0)
    ...
    axs[0].plot(r_vals, sz_values, 'b-o', linewidth=2, markersize=8)
    axs[0].set_ylabel(r'$\langle s_z \rangle$', fontsize=14)
    axs[0].grid(True, alpha=0.3)

    axs[1].plot(r_vals, s2_values / 2, 'r-o', linewidth=2, markersize=8)
